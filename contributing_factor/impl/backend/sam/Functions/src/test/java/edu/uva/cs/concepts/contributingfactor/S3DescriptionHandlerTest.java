...
import edu.uva.cs.concepts.lambda.concrete.S3DescriptionHandler;
...
public class S3DescriptionHandlerTest {
    ...
    private static final String DESCRIPTION_KEY = "contributing-factor/description.json";
    ...
    public void setup() {
        ...
        descriptionMap.put("patient_unnecessary_treatments", "Feeling pressured to give unnecessary/inappropriate test and/or treatments");
        s3Client.putObject(
                builder -> builder.bucket(BUCKET_NAME).key(DESCRIPTION_KEY),
                RequestBody.fromString(JacksonHelper.toJson(descriptionMap))
        );
    }
    ...
    public void test_description() {
        ...
        S3DescriptionHandler handler = new S3DescriptionHandler();
        APIGatewayProxyResponseEvent response = handler.handleRequest(event, new MockContext());
        assertEquals(200, response.getStatusCode());
        assertEquals("\"Feeling pressured to give unnecessary/inappropriate test and/or treatments\"", response.getBody());
    }
